...
class Zombie(poc_grid.Grid):
    ...
    def compute_distance_field(self, entity_type):
        ...
        if entity_type==ZOMBIE:
            for ele in self._zombie_list:
                boundary.enqueue(ele)
        else:
            for ele in self._human_list:
                boundary.enqueue(ele)
    ...
    def move_humans(self, zombie_distance):
        ...
        for cell in self._human_list:
            neigh=self.eight_neighbors(cell[0],cell[1])
            neigh.append(cell)
            maxd=0
            for cel in neigh:
                
                dist=zombie_distance[cel[0]][cel[1]]
                if dist>maxd:
                    maxd=dist
                    tup=cel
            temp.append(tup)
    ...
    def move_zombies(self, human_distance):
        ...
        for cell in self._zombie_list:
            neigh=self.four_neighbors(cell[0], cell[1])
            neigh.append(cell)
            mind=float("inf")
            for cel in neigh:
                dist=human_distance[cel[0]][cel[1]]
                if self.is_empty(cel[0],cel[1]):
                    if dist<mind:
                        mind=dist
                        tup=cel
            temp.append(tup)
